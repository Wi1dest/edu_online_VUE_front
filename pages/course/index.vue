...
export default {
  ...
  created() {
    this.getCoursePageList(), this.getAllSubjectList();
  },
  methods: {
    getCoursePageList() {
      course.getCoursePage(1, 8, this.searchObj).then((response) => {
        this.data = response.data.data;
      });
    },
    getAllSubjectList() {
      course.getAllSubjectList().then((response) => {
        this.subjectNestedList = response.data.data;
      });
    },
    gotoPage(page) {
      course.getCoursePage(page, 8, this.searchObj).then((response) => {
        this.data = response.data.data;
      });
    },
    //4 点击某个一级分类，查询对应二级分类
    searchOne(subjectParentId, index) {
      ...
      for (let i = 0; i < this.subjectNestedList.length; i++) {
        var subjectNested = this.subjectNestedList[i];
        if (subjectParentId == subjectNested.id) {
